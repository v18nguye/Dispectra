...
x_SFW_blasso_c = x_SFW_blasso;
v = zeros(length(x_SFW_blasso),4);
i = zeros(1,4);
% find the largest coefficient.
[m1,i1] = max(x_SFW_blasso_c);
% assign the i1th value to the minimum value of the vector
x_SFW_blasso_c(i1) = min(x_SFW_blasso_c);
v(i1,1) = m1;
i(1,1) = i1;

% find the second largest coefficient.
[m2,i2] = max(x_SFW_blasso_c);
%...
x_SFW_blasso_c(i2) = min(x_SFW_blasso_c);
v(i2,2) = m2;
i(1,2) = i2;
% find the third largest coefficient.
[m3,i3] = max(x_SFW_blasso_c);
%...
x_SFW_blasso_c(i3) = min(x_SFW_blasso_c);
v(i3,3) = m3;
i(1,3) = i3;
% find the fourth largest coefficient.
[m4,i4] = max(x_SFW_blasso_c);
%...
x_SFW_blasso_c(i4) = min(x_SFW_blasso_c);
v(i4,4) = m4;
i(1,4) = i4;
...
shading flat
title(['H_T[',num2str(param_SFW_blasso(1,k)),' ',num2str(param_SFW_blasso(2,k)),'] c_theta[',num2str(param_SFW_blasso(3,k)),' ',num2str(param_SFW_blasso(4,k)),'] - a: ', num2str(x_SFW_blasso(i(1,k),1))]); 
end
