function [param_est , x , fc_blasso , fc_lasso , fc_lassoDual ] = SFW2d( y , opts)

% This code is an implementation of the Sliding-Frank-Wolfe algorithm [1] for the
% blasso problem for the case in which these are two estimated parameters.
...
%       param_grid : the 2*N vector of the discretized atom parameters subspace
...
for iter = 1 : opts.maxIter
    ...
    [ out_atom_selection ] = atom_selection_2d( residual , opts );
...
x1 = [param(1,:)';abs(coeff);angle(coeff)]; % for the first parameter.
x2 = [param(2,:)';abs(coeff);angle(coeff)]; % .. .
X = [x1 x2];


A1 = [ -eye(n) , zeros(n,2*n), zeros(n,3*n) ;... % for the first parameter.
    eye(n) , zeros(n,2*n), zeros(n,3*n) ;...
    zeros(n) , -eye(n) , zeros(n,n), zeros(n, 3*n)];
b1 = [ -ones(n,1)*B(1,1) ; ones(n,1)*B(2,1) ; zeros(n,1) ];

A2 = [ zeros(n,3*n),-eye(n) , zeros(n,2*n) ;... % ...
    zeros(n,3*n), eye(n) , zeros(n,2*n) ;...
    zeros(n,3*n), zeros(n) , -eye(n) , zeros(n,n)];
b2 = [ -ones(n,1)*B(1,2) ; ones(n,1)*B(2,2) ; zeros(n,1) ];
...
fobj = @(X0) joint_cost( y , X0 , lambda , atom , datom , cplx );
...
if(cplx)
    ...
    coeff = real(X(n+1:2*n,1).*exp(1i*X(2*n+1:3*n,1)));
end
t = norm(coeff,1);
...
for i = 1: length(coeff)
    ...
    coeff2d = [coeff2d coeff(i,1)*ones(1,2)];
    
end
grad_theta = -real(res'*dA*diag(coeff2d));
